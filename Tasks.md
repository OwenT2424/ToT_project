...
### Task 1 — Stub the Routes (`src/routes/index.js`) - ZAYNAH

**Goal:** Every endpoint exists on the server and responds. No real logic yet.

Register all routes with a temporary placeholder handler, example:
...
- [ ] **1b — Story routes**
  ...
  GET  /stories             // to fetch all stories
  GET  /stories/:storyId    // to fetch a story by story id
  ```

- [ ] **1c — Chapter routes**
  ```javascript
  GET  /stories/:storyId/chapters               // fetch all the chapters
  GET  /stories/:storyId/chapters/:chapterId    // fetch a chapter by id
  POST /stories/:storyId/chapters               // to create a chapter
  POST /stories/:storyId/chapters/:chapterId    // to update a chapter by its id
  ```

**API Response for all of them:** `{ "message": "TODO" }`.
...
- Branch Name: api-branch
- Staging file: `src/routes/index.js` only
- Always pull before pushing

---

### Task 2 — Create the Models - KRIS

**Goal:** A clean database layer that the controllers will call. No HTTP code here. Each model is a plain JS object exported from its file:
...
const id = uuidv4();
await pool.execute("INSERT INTO TableName (col1, col2) VALUES (?, ?)", [
  val1,
  val2,
]);
```
...
**Note that all of these functions need to be asynchronous.** Example, `async create({ username, email, password_hash }){ ... }`. For the SQL statement execution, it has to be proceeded with `await`. See the example above.
...
**Note that all of these functions need to be asynchronous.** For the SQL statement execution, it has to be proceeded with `await`. See the example above.
...
### Task 3 — Implement the Controllers - OWEN
...
The logged-in user's ID lives at `req.session.userId`. Controllers can read it freely — it will be _protected_ in Phase 5.
...
Imports needed (Done by Tejas): `const Story = require("../models/storyModel");`
...
Imports needed (**Done by Tejas**):

**Requirements:** Kris has to be done with his work.
...
### Task 4 — Create the Auth Middleware (`src/middleware/requireAuth.js`) - TEJAS
...
#### [ ] 4a — Auth Controller (`src/controllers/authController.js`)
